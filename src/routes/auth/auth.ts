  ...
  try {
    ...
    if (name && email && password && cnic && location && description && role) {
      ...
      if (role == "expert" && !experties) {
        return res.status(400).json({ message: "Experties Required" });
      }
      if (existingUser) {
        ...
        if (validator.isEmail(email)) {
          ...
          if (!isLowercaseValid || !isMinLengthValid || !isUppercaseValid) {
            return res.status(402).json({ message: "Password Problem" });
          }
          ...
          if (!isCNICLength || cnic.length < 13 || cnic.length > 13) {
            return res.status(400).json({ message: "CNIC Problem" });
          }
      ...
      res.status(400).json({ message: "Fill All" });
    }
  } catch (error: any) {
    console.log(error);
...
router.post("/enterOtp", async (req, res) => {
  ...
  if (otpE) {
    ...
    return res.status(401).json({ message: "OTP failed" });
  }
});
